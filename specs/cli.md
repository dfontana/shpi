...
`shpi` is a single binary with two roles: a receiver (`start`/`stop`/`status`/`watch`)
...
### `shpi start [--port <PORT>] [--log <PATH>] <user@host>...`
...
- `--port <PORT>` — TCP port used for both the local listener and the remote forwarded
  port. Default: `9292`.
- `--log <PATH>` — daemon log file location. Default: a fixed path under the shpi
  data directory.

Fails with a distinct, clear error when:
- A shpi daemon is already running.
- The port is already bound by a non-shpi process.
...
### `shpi send [--port <PORT>] [--name <NAME>] <message>`
